...
class TestContinue:
    def test_continue_with_message_calls_single(self):
        with patch("gemiterm.cli.load_cookies") as mock_cookies:
            ...
            with patch("gemiterm.cli.GeminiClient") as mock_client_class:
                ...
                mock_client.continue_chat.assert_called_once_with("cid123", "Hello AI")
    ...
    def test_continue_cookie_expired(self):
        with patch("gemiterm.cli.load_cookies") as mock_cookies:
...
class TestExport:
    def test_export_auto_filename_in_cwd(self, tmp_path):
        with patch("gemiterm.cli.load_cookies") as mock_cookies:
            ...
            with patch("gemiterm.cli.GeminiClient") as mock_client_class:
                ...
                mock_client_class.return_value = mock_client
                with patch("gemiterm.cli.Path.cwd", return_value=tmp_path):
                    runner = CliRunner()
    ...
    def test_export_authentication_error(self):
        with patch("gemiterm.cli.load_cookies") as mock_cookies:
            from gemiterm.exceptions import AuthenticationError
            mock_cookies.side_effect = AuthenticationError("Not authenticated")
            runner = CliRunner()
            result = runner.invoke(cli, ["export", "cid123"])
            assert result.exit_code == 2
    ...
    def test_export_all_progress_bar(self, tmp_path):
        with patch("gemiterm.cli.load_cookies") as mock_cookies:
            ...
            with patch("gemiterm.cli.GeminiClient") as mock_client_class:
                ...
                runner = CliRunner()
                result = runner.invoke(
                    cli, ["export-all", "--output-dir", str(tmp_path)]
                )
                assert result.exit_code == 0
